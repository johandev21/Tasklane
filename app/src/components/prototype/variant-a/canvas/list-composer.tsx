...
export function ListComposer({
  ...
  // Handle click outside to cleanly close the composer
  useEffect(() => {
    if (!isOpen) return
  ...
  const handleSubmit = (e?: FormEvent) => {
    ...
    if (newCardText.trim()) {
      ...
      setNewCardText('')
      // Keep focused for rapid batch sequential creation
      setTimeout(() => {
        textareaRef.current?.focus()
      }, 0)
    }
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="flex w-full items-center gap-1.5 rounded-xl px-2.5 py-1.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted/70 hover:text-foreground group"
    >
